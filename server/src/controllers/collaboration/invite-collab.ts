...
const invitationToCollaborate = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const author = project.user_id as any;
    ...
    try {
      ...
      return sendResponse(
        res,
        500,
        (emailError as Error).message || 'Failed to send invitation email'
      );
    }
    ...
    // If email was not sent, return error
    return sendResponse(res, 500, 'Failed to send invitation email');
  } catch (err) {
    return sendResponse(
      res,
      500,
