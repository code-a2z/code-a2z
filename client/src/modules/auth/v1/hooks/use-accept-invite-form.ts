...
export function useAcceptInviteForm() {
    ...
    getAcceptInviteInfo(token)
      .then(res => {
        ...
        if (res.status === 'success' && res.data?.org_name) {
          ...
          setError(res.message ?? 'Invite not found or expired');
        }
      })
      .catch((err: ErrorResponse) => {
        if (cancelled) return;
        const msg = err?.response?.data?.message ?? 'Failed to load invite';
        setError(msg);
      })
      .finally(() => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await postAcceptInvite({ token, fullname, password });

      if (response.status !== 'success' || !response.data) {
        addNotification({
          message: response.message ?? 'Accept invite failed.',
          type: 'error',
        });
        setError(response.message ?? 'Accept invite failed.');
        return;
      }
